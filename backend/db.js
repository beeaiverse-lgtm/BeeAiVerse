...
CREATE TABLE IF NOT EXISTS support_tickets (
  ...
  user_id INTEGER,                       -- NULL for guest (pre-login) tickets
  guest_name TEXT,                       -- set when user_id is NULL
  guest_email TEXT,                      -- set when user_id is NULL
  guest_user_id_hint TEXT,               -- BAV ID / email hint provided by guest
...
const migrations = [
  ...
  // Guest (pre-login) support tickets — user_id may be NULL for locked-out users
  `ALTER TABLE support_tickets ADD COLUMN guest_name TEXT`,
  `ALTER TABLE support_tickets ADD COLUMN guest_email TEXT`,
  `ALTER TABLE support_tickets ADD COLUMN guest_user_id_hint TEXT`,
  ...
  // Records that the user ticked the Terms & Conditions checkbox at signup.
  `ALTER TABLE users ADD COLUMN terms_accepted INTEGER DEFAULT 0`,
  `ALTER TABLE users ADD COLUMN terms_accepted_at TEXT`,
  // Forgot-password: one-time email OTP (hashed) + expiry. Cleared after use.
  `ALTER TABLE users ADD COLUMN reset_otp_hash TEXT`,
  `ALTER TABLE users ADD COLUMN reset_otp_expires TEXT`
];
