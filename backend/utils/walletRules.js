...
// 18th/36th-month rule — used so admins can test the withdrawal flow
// without waiting for a real eligibility window.
function getPrincipalEligibility(db, userId, now = new Date(), bypassLock = false) {
  ...
  for (const d of deposits) {
    ...
    if (monthsElapsed === 18 || monthsElapsed === 36) {
      eligibleNow += remaining;
      lots.push({ deposit_id: d.id, available: remaining, monthsElapsed });
    } else if (monthsElapsed < 18) {
      const windowStart = addMonths(approvedAt, 18);
      if (!nextWindowDate || windowStart < nextWindowDate) nextWindowDate = windowStart;
    } else if (monthsElapsed > 18 && monthsElapsed < 36) {
      const windowStart = addMonths(approvedAt, 36);
      if (!nextWindowDate || windowStart < nextWindowDate) nextWindowDate = windowStart;
    }
    // monthsElapsed > 36 with remaining balance: both windows already passed
    // for this lot, nothing further scheduled for it.
  }
...
function reservePrincipal(db, userId, amount, now = new Date(), bypassLock = false) {
  const { eligibleNow, lots } = getPrincipalEligibility(db, userId, now, bypassLock);
