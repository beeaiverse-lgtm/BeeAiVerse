...
router.get('/users', (req, res) => {
  const rows = db.prepare(`
    SELECT u.id, u.user_code, u.name, u.email, u.phone, u.sponsor_id, s.user_code AS sponsor_code,
           ...
           COALESCE((SELECT SUM(d.amount) FROM deposits d WHERE d.user_id = u.id AND d.status = 'approved'), 0) AS total_deposit_amount,
           COALESCE((SELECT SUM(w.amount) FROM withdrawals w WHERE w.user_id = u.id AND w.status != 'rejected'), 0) AS total_withdrawal_amount,
           COALESCE((SELECT SUM(il.amount) FROM income_log il WHERE il.user_id = u.id AND il.type = 'level'), 0) AS level_income_deduction
    FROM users u LEFT JOIN users s ON s.id = u.sponsor_id
    WHERE u.role='user' ORDER BY u.id DESC
  `).all();

  // Principal Amount = Total Deposit Amount - (Total Withdrawal Amount + Level income deduction)
  rows.forEach(u => {
    u.principal_amount = Math.round((u.total_deposit_amount - (u.total_withdrawal_amount + u.level_income_deduction)) * 100) / 100;
  });
...
router.get('/support', (req, res) => {
  ...
  const rows = db.prepare(`
    SELECT t.*,
           u.user_code, u.name AS user_name, u.email AS user_email,
           -- fall back to guest fields when user_id is NULL
           COALESCE(u.name,  t.guest_name)  AS display_name,
           COALESCE(u.email, t.guest_email) AS display_email,
...
// ---------- ADMIN: RESET USER GOOGLE AUTHENTICATOR (TOTP) ----------
// Used when a user opens a support ticket because they lost access to their
// authenticator app. Admin clears the TOTP secret so the user is forced to
// re-scan a new QR code on next login.
router.post('/users/:id/reset-totp', (req, res) => {
  const user = db.prepare(`SELECT id, user_code, role FROM users WHERE id = ?`).get(req.params.id);
  if (!user) return res.status(404).json({ error: 'User not found' });
  if (user.role === 'admin') return res.status(400).json({ error: 'Cannot reset admin TOTP from this panel' });

  db.prepare(`UPDATE users SET totp_secret = NULL, totp_enabled = 0 WHERE id = ?`).run(user.id);

  // Log the action
  db.prepare(`INSERT INTO income_log (user_id, type, amount, note) VALUES (?, 'admin_action', 0, ?)`)
    .run(user.id, `Google Authenticator reset by admin (support action)`);

  res.json({ message: `Google Authenticator reset for ${user.user_code}. User will be prompted to set up 2FA on next login.` });
});

// ---------- ADMIN: RESET USER PASSWORD ----------
// Used when a user cannot log in and opens a support ticket. Admin sets a
// temporary password; the user should change it immediately after logging in.
router.post('/users/:id/reset-password', (req, res) => {
  ...
  const user = db.prepare(`SELECT id, user_code, role FROM users WHERE id = ?`).get(req.params.id);
  if (!user) return res.status(404).json({ error: 'User not found' });
  if (user.role === 'admin') return res.status(400).json({ error: 'Cannot reset admin password from this panel' });
  ...
  // Log the action
  db.prepare(`INSERT INTO income_log (user_id, type, amount, note) VALUES (?, 'admin_action', 0, ?)`)
    .run(user.id, `Password reset by admin (support action)`);

  res.json({ message: `Password reset for ${user.user_code}. Please share the new temporary password with the user securely.` });
});

// ---------- BSCSCAN TRANSACTION CHECKER ----------
// Proxies a BSCScan API call server-side so the API key never leaks to the
// browser and CORS is not an issue. Admin clicks "Check Txn" → this endpoint
// fetches the BEP20 USDT transfer details and returns a clean summary.
// The admin still manually approves or rejects — nothing is automated here.

const USDT_BEP20_CONTRACT = '0x55d398326f99059fF775485246999027B3197955'; // BSC USDT (Binance-Peg)

router.get('/deposits/:id/check-txn', async (req, res) => {
  ...
  if (!dep.txn_ref) return res.status(400).json({ error: 'No transaction hash on this deposit' });

  const apiKey = process.env.BSCSCAN_API_KEY || '';
  const txHash = dep.txn_ref.trim();

  try {
    // Fetch both the plain TX receipt (for BNB/status) and the BEP20 token
    // transfer logs so we can read the USDT amount and recipient address.
    const baseUrl = 'https://api.bscscan.com/api';

    const [txRes, tokenRes, depositAddrRow] = await Promise.all([
      fetch(`${baseUrl}?module=transaction&action=gettxreceiptstatus&txhash=${txHash}&apikey=${apiKey}`),
      fetch(`${baseUrl}?module=account&action=tokentx&contractaddress=${USDT_BEP20_CONTRACT}&txhash=${txHash}&apikey=${apiKey}`),
      Promise.resolve(db.prepare(`SELECT value FROM settings WHERE key = 'deposit_address_BEP20'`).get()),
    ]);

    const txData    = await txRes.json();
    const tokenData = await tokenRes.json();

    const depositAddress = (depositAddrRow?.value || '').toLowerCase();

    // Receipt status: "1" = success, "0" = failed / reverted
    const receiptStatus = txData?.result?.status;

    // Find the USDT transfer in the token log that goes TO our deposit address
    ...
    const match = transfers.find(t =>
      ...
      (t.to?.toLowerCase() === depositAddress || !depositAddress) // always show if no address configured
    );

    // Also accept any USDT transfer in this tx even if to-address differs
    // (admin can visually verify) — just flag it
    const anyTransfer = match || transfers[0] || null;
    ...
    if (anyTransfer) {
      const decimals = parseInt(anyTransfer.tokenDecimal || '18', 10);
      usdtAmount  = parseFloat(anyTransfer.value) / Math.pow(10, decimals);
      toAddress   = anyTransfer.to;
      fromAddress = anyTransfer.from;
      tokenSymbol = anyTransfer.tokenSymbol;
      blockTime   = anyTransfer.timeStamp ? new Date(parseInt(anyTransfer.timeStamp) * 1000).toISOString() : null;
      isToOurWallet = depositAddress ? toAddress?.toLowerCase() === depositAddress : null;
    }

    res.json({
      txHash,
      txSuccess: receiptStatus === '1' ? true : receiptStatus === '0' ? false : null,
      usdtAmount,
      tokenSymbol,
      fromAddress,
      toAddress,
      depositAddress: depositAddrRow?.value || null,
      isToOurWallet,
      blockTime,
      depositRequested: dep.amount,
      bscscanUrl: `https://bscscan.com/tx/${txHash}`,
      raw: { txData, tokenData },
    });
  } catch (err) {
    console.error('BSCScan check failed:', err);
    res.status(502).json({ error: 'Could not reach BSCScan. Check your BSCSCAN_API_KEY env variable or try again.' });
  }
});
