...
router.post('/register', (req, res) => {
  try {
    const { name: rawName, email, password, sponsorCode, sponsorEmail, side, termsAccepted } = req.body;
    ...
    // Must explicitly tick the Terms & Conditions checkbox on the register form.
    ...
    const info = db.prepare(`
      INSERT INTO users (user_code, name, email, password, role, sponsor_id, parent_id, position, wallet_balance, status, terms_accepted, terms_accepted_at)
      VALUES (?, ?, ?, ?, 'user', ?, ?, ?, 0, 'inactive', 1, datetime('now'))
...
// Email OTP password reset has been removed. Password changes are handled
// through the Profile page using Google Authenticator (TOTP) verification.
// If an admin is locked out, the password must be reset directly in the database.

// ---------- GUEST SUPPORT TICKET (no auth required) ----------
// Allows locked-out users to raise a ticket without being logged in.
const GUEST_TICKET_TYPES = {
  cant_login:         4,
  lost_2fa:           2,   // high priority — usually a blocker
  forgot_password:    3,
  account_blocked:    2,
  other:              7,
};

router.post('/guest-support', (req, res) => {
  const { guest_name, guest_email, guest_user_id_hint, ticket_type, subject, message } = req.body;
  ...
  const type   = GUEST_TICKET_TYPES[ticket_type] !== undefined ? ticket_type : 'cant_login';
  const priority = GUEST_TICKET_TYPES[type] ?? 4;
  ...
  try {
    const tx = db.transaction(() => {
      const info = db.prepare(`
        INSERT INTO support_tickets
          (ticket_no, user_id, guest_name, guest_email, guest_user_id_hint,
           subject, status, ticket_type, priority)
        VALUES (?, NULL, ?, ?, ?, ?, 'open', ?, ?)
      `).run(ticketNo,
             guest_name.trim(),
             guest_email.trim().toLowerCase(),
             (guest_user_id_hint || '').trim(),
             subject.trim(),
             type,
             priority);
    ...
    const id = tx();
    res.json({ message: 'Ticket submitted successfully.', ticketNo, id });
  } catch (err) {
    console.error(err);
