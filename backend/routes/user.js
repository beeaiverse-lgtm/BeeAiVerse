...
router.get('/totp/status', (req, res) => {
  try {
    ...
    const generated = speakeasy.generateSecret({ name: `Bee AI Verse (${req.user.email})`, issuer: 'Bee AI Verse' });
...
// Admin "testing mode" switch (Admin > Settings) that bypasses the 18th/
// 36th-month Principal rule and the last-day-of-month Income rule, so
// withdrawals can be tested end-to-end at any time. Off by default.
...
router.get('/me', (req, res) => {
  ...
  const principal = getPrincipalEligibility(db, user.id, now, bypassLock);
...
router.post('/withdrawals', (req, res) => {
  try {
    ...
    if (wallet_type === 'income') {
      ...
      const reserved = reservePrincipal(db, user.id, amount, now, bypassLock);
      if (!reserved.ok) {
        const { nextWindowDate } = getPrincipalEligibility(db, user.id, now, bypassLock);
        ...
        const windowMsg = nextWindowDate
          ...
          : '';
        return res.status(400).json({
          error: `Principal Wallet withdrawals are only allowed at the 18th or 36th month after a deposit is approved. ${eligibleMsg}${windowMsg}`
        });
      }
...
// ---------- INCOME WALLET REPORT (month-wise) ----------
// Every admin "Bulk Income % Adjustment" run writes one row per credited
// deposit into bonus_credits. Group those by month so the user can see,
// per month, how much was credited to their Income Wallet and from how
// many deposits.
router.get('/income-report', (req, res) => {
  ...
  res.json({ income_report: rows });
});

// Drill-down: individual credits for one month (YYYY-MM), each tied back
// to the deposit that earned it.
router.get('/income-report/:month', (req, res) => {
  const month = req.params.month;
  if (!/^\d{4}-\d{2}$/.test(month)) {
    return res.status(400).json({ error: 'Month must be in YYYY-MM format' });
  }
  const rows = db.prepare(
    `SELECT bc.*, d.amount AS deposit_amount
     FROM bonus_credits bc
     JOIN deposits d ON d.id = bc.deposit_id
     WHERE bc.user_id = ? AND bc.month = ?
     ORDER BY bc.id DESC`
  ).all(req.user.id, month);
  res.json({ month, credits: rows });
});
