...
// Returns the full sponsor-chain downline with depth level (level 1 = direct, 2+ = indirect)
router.get('/indirect-team', (req, res) => {
  const rows = db.prepare(`
    WITH RECURSIVE chain(id, depth) AS (
      SELECT id, 1 FROM users WHERE sponsor_id = ?
      UNION
      SELECT u.id, c.depth + 1
      FROM users u JOIN chain c ON u.sponsor_id = c.id
    )
    SELECT u.id, u.user_code, u.name, u.email, u.phone, u.position,
           COALESCE(u.main_wallet, 0) AS main_wallet,
           COALESCE(u.referral_wallet, 0) AS referral_wallet,
           (COALESCE(u.main_wallet, 0) + COALESCE(u.referral_wallet, 0)) AS wallet_balance,
           u.status, u.created_at, u.sponsor_id, c.depth,
           s.name AS sponsor_name, s.user_code AS sponsor_code
    FROM chain c
    JOIN users u ON u.id = c.id
    LEFT JOIN users s ON s.id = u.sponsor_id
    WHERE c.depth >= 2
    ORDER BY c.depth ASC, u.id ASC
  `).all(req.user.id);
  res.json(rows);
});
...
// ---------- CHANGE PASSWORD (authenticated, requires TOTP) ----------
// Allows any logged-in user (including admin) to change their password by
// confirming the current password + a live Google Authenticator code.
router.post('/change-password', (req, res) => {
  try {
    const { currentPassword, newPassword, totpCode } = req.body;
    if (!currentPassword || !newPassword || !totpCode) {
      return res.status(400).json({ error: 'Current password, new password and authenticator code are required' });
    }
    if (newPassword.length < 6) {
      return res.status(400).json({ error: 'New password must be at least 6 characters' });
    }

    const user = db.prepare('SELECT id, password, totp_secret, totp_enabled FROM users WHERE id = ?').get(req.user.id);
    ...
    const { verifyToken } = require('../utils/totp');
    if (!verifyToken(totpCode, user.totp_secret)) {
      return res.status(400).json({ error: 'Incorrect or expired authenticator code' });
    }
